...
class ElatsEmailApi extends ApiModel {

	public function __construct($model) {
		$this->setModel($model);
		$this->setTitle("E-lats API");
		$this->setContent($this->_getContent());
		
		if (\Yii::$app->getRequest()->get("task") == "elats-email") {
			$this->_post();
	...
	private function _getContent() {
		...
		if ($this->getModel()->extra && $this->getModel()->extra->bank_account_statement) {
			$sent = Changes::find()->where(["type_id" => $this->getModel()->id, "type" => Loan::TYPE, "attr" => "actions_41"])->one();
			if ($sent) {
				$html .= "<p>Already sent data.</p>";
			} else {
				$html .= Html::beginForm("?task=elats-email", "get", ["id" => ""]);
	...
	private function _post() {
		...
		if (!$sent) {
			...
			$text .= "Work place: ".$this->getModel()->extra->car_workplace."<br />";
			$text .= "Auto: ".($this->getModel()->extra->car_description ? $this->getModel()->extra->car_description : "BENTLEY AZURE")."<br />";
			...
			$mailer = \Yii::$app->mailer->compose()
    			...
    			$file = "https://www.server.lv/?file=".str_replace("$", "%24", $this->getModel()->extra->bank_account_statement)."&f_preview=yes";
